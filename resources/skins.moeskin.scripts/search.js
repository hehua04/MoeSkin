...
function renderSearchClearButton( input ) {
	...
	clearButton.classList.add( 'moeskin-search__formButton', 'moeskin-search__clear' );
...
function initSearch( window ) {
	const searchConfig = require( './config.json' ).wgMoeSkinEnableSearch,
		checkbox = document.getElementById( 'moeskin-search__checkbox' ),
	...
	if ( searchConfig ) {
		...
		loadSearchModule( input, 'skins.moeskin.search', () => {
