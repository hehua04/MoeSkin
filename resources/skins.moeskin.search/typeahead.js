const
	PREFIX = 'moeskin-typeahead',
	SEARCH_LOADING_CLASS = 'moeskin-loading',
	ITEM_CLASS = `${PREFIX}__item`,
	ACTIVE_CLASS = `${ITEM_CLASS}--active`,
	HIDDEN_CLASS = `${ITEM_CLASS}--hidden`;

/**
 * Config object from getMoeSkinSearchResourceLoaderConfig()
 */
		...
		if ( results.length > 0 ) {
			...
			const getRedirectLabel = ( title, matchedTitle ) => {
				...
				if ( matchedTitle && isRedirectUseful() ) {
					html = `<div class="${PREFIX}__labelItem" title="${mw.message( 'search-redirect', matchedTitle ).plain()}">
							<span class="moeskin-ui-icon mw-ui-icon-wikimedia-articleRedirect"></span>
			...
			updateMenuItem(
				placeholder,
				{
					...
					type: 'placeholder',
					size: 'lg',
					title: mw.message( 'moeskin-search-noresults-title', htmlSafeSearchQuery ).text(),
					desc: mw.message( 'moeskin-search-noresults-desc' ).text()
				}
			);
	...
	const getTools = () => {
		...
		const updateToolItem = ( data ) => {
			const
				...
				query = `<span class="moeskin-typeahead__query">${htmlSafeSearchQuery}</span>`,
		...
		updateToolItem( {
			...
			icon: 'articleSearch',
			msg: hasQuery ? 'moeskin-search-fulltext' : 'moeskin-search-fulltext-empty'
		} );
		...
		if ( config.isMediaSearchExtensionEnabled ) {
			updateToolItem( {
				...
				icon: 'imageGallery',
				msg: hasQuery ? 'moeskin-search-mediasearch' : 'moeskin-search-mediasearch-empty'
			} );
		}
	};
	...
	if ( hasQuery ) {
		...
		updateMenuItem(
			placeholder,
			{
				...
				desc: mw.message( 'moeskin-search-empty-desc' ).text()
			}
		);
		placeholder.classList.remove( HIDDEN_CLASS );
		updateActiveIndex();
...
function initTypeahead( searchForm, input ) {
	const EXPANDED_CLASS = 'moeskin-search__card--expanded';

	const
		template = mw.template.get(
			'skins.moeskin.search',
			'resources/skins.moeskin.search/templates/typeahead.mustache'
		),
		data = {
			'msg-searchsuggest-search': mw.message( 'searchsuggest-search' ).text(),
			'msg-moeskin-search-empty-desc': mw.message( 'moeskin-search-empty-desc' ).text()
		};
