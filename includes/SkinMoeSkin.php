...
use Html;
use Linker;
use MediaWiki\Skins\MoeSkin\Partials\BodyContent;
use MediaWiki\Skins\MoeSkin\Partials\Drawer;
use MediaWiki\Skins\MoeSkin\Partials\Footer;
use MediaWiki\Skins\MoeSkin\Partials\Header;
use MediaWiki\Skins\MoeSkin\Partials\Logos;
use MediaWiki\Skins\MoeSkin\Partials\PageTools;
use MediaWiki\Skins\MoeSkin\Partials\Tagline;
use MediaWiki\Skins\MoeSkin\Partials\Theme;
use MediaWiki\Skins\MoeSkin\Partials\Title;
use Sanitizer;
use SkinMustache;

/**
 * Skin subclass for MoeSkin
 * @ingroup Skins
*/
class SkinMoeSkin extends SkinMustache {
	use GetConfigTrait;
	...
	*/
	...
	/**
	 * @return array Returns an array of data used by MoeSkin.
	 * @throws MWException
	*/
	public function getTemplateData(): array {
		...
		$data += [
			...
			'data-header' => [
				...
				'html-moeskin-jumptotop' => $this->msg( 'moeskin-jumptotop' )->text() . ' [home]',
			],
	...
	*/
	public function getDefaultModules() {
		...
		$modules['content'] = array_diff( $modules['content'], [
			// MoeSkin provides its own implementation. Loading this will break display.
			'mediawiki.toc'
		] );
	...
	*/
	private function buildSkinFeatures( array &$options ) {
		...
		if ( $title !== null ) {
			// Load MoeSkin collapsible sections modules if enabled
				$options['scripts'][] = 'skins.moeskin.scripts.sections';
				$options['styles'][] = 'skins.moeskin.styles.sections';
				$options['styles'][] = 'skins.moeskin.icons.sections';
		}

		// Table of content highlight
		$options['styles'][] = 'skins.moeskin.styles.toc';

		// Drawer sitestats
		$options['styles'][] = 'skins.moeskin.styles.sitestats';
		...
		if (
			...
			$options['styles'][] = 'skins.moeskin.styles.debug';
		}
	}
}
